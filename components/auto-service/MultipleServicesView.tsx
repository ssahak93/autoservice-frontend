...
/**
 * View for managing multiple auto services
 * Shows selector and allows switching between services
 */
export function MultipleServicesView() {
  ...
  const currentServiceId = useMemo(() => {
    if (selectedAutoServiceId && ownedServices.some((s) => s.id === selectedAutoServiceId)) {
      return selectedAutoServiceId;
    }
    return ownedServices[0]?.id || null;
  }, [selectedAutoServiceId, ownedServices]);

  const { profile, isLoading, isProfileNotFound, publishMutation } =
    useAutoServiceProfile(currentServiceId);
  ...
  // If no service is selected, or if the selected service doesn't have a profile,
  // show the list of services and potentially a create profile prompt.
  if (!currentServiceId || isProfileNotFound) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        <motion.div
        ...
        >
          <div className="mx-auto max-w-6xl">
            ...
            {/* Show create profile form if a service is selected but has no profile */}
            {currentServiceId && isProfileNotFound && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
      ...
      <div className="container mx-auto px-4 py-6 lg:py-8">
        ...
        >
          <BlockedServiceBanner
            autoServiceId={currentServiceId}
          ...
          <div className="glass-light rounded-2xl p-6 lg:p-8">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === 'info' && <AutoServiceEditor autoServiceId={currentServiceId} />}
