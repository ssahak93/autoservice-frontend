...
export function AutoServicesList() {
  ...
  // Use custom hook for fetching available auto services (SOLID - Single Responsibility)
  ...
  const incompleteServices = useMemo(
    () => (Array.isArray(ownedServices) ? ownedServices.filter((s) => s && !s.hasProfile) : []),
    [ownedServices]
  );
  ...
  const handleSelectService = useCallback(
    ...
    [router, setSelectedAutoServiceId]
  );

  const handleDelete = useCallback((serviceId: string, hasProfile: boolean) => {
    setDeleteConfirm({ isOpen: true, serviceId, hasProfile });
  }, []);

  // Use custom hook for delete mutation (SOLID - Single Responsibility)
  const deleteMutation = useDeleteAutoService();
            ...
            <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
              {incompleteServices.map((service, index) => (
                <ServiceCard
                  ...
                  index={index}
                  variant="incomplete"
                  isSelected={selectedAutoServiceId === service.id}
