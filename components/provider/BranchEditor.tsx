...
export function BranchEditor({ branch }: BranchEditorProps) {
  const t = useTranslations('myService.profile');
  const [selectedRegionId, setSelectedRegionId] = useState<string>(branch.regionId || '');
  ...
  const {
    ...
    defaultValues: {
      ...
      yearsOfExperience: branch.yearsOfExperience,
      address: branch.address,
      regionId: branch.regionId || '',
      communityId: branch.communityId || '',
      latitude: Number(branch.latitude),
      longitude: Number(branch.longitude),
      phoneNumber: parsePhoneFromBackend(branch.phoneNumber),
      maxVisitsPerDay: branch.maxVisitsPerDay,
      serviceTypes: branch.services?.map((s) => s.serviceType.id) || [],
    },
  });

  // Update selectedRegionId when branch.regionId changes
  useEffect(() => {
    if (branch.regionId) {
