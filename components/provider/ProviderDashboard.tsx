...
/**
 * Dashboard for managing a single provider
...
export function ProviderDashboard({ providerId }: ProviderDashboardProps) {
  ...
  const { branch, isLoading, isBranchNotFound, publishMutation } = useProviderBranch(providerId);
  const { getSelectedProvider } = useProviderStore();
  const { user } = useAuth();

  const variants = getAnimationVariants();

  // Get provider from user.providers (available immediately after reload)
  const userProvider = user?.providers?.find((p) => p.id === providerId);
  ...
  // Show create branch form if branch doesn't exist
  if (isBranchNotFound) {
    return (
          ...
          <div className="mx-auto max-w-5xl">
            ...
            >
              ...
              <div className="mt-8 border-t border-gray-200 pt-8 dark:border-gray-700">
                <CreateBranchEditor providerId={providerId} />
              </div>
            </motion.div>
          </div>
        </motion.div>
  ...
  // Get provider name - use multiple sources with priority:
  // 1. user.providers (available immediately after reload)
  // 2. branch data (available after branch loads)
  // 3. store data (fallback)
  const selectedProvider = getSelectedProvider();

  const getProviderName = () => {
    // First try user.providers (available immediately after reload)
    if (userProvider) {
      ...
      if (provider.serviceType === 'company' && provider.companyName) {
    ...
    // Second try branch data (available after branch loads)
    if (provider) {
      if (provider.serviceType === 'company' && provider.companyName) {
        return provider.companyName;
      }
  ...
  // Use provider avatar - prefer user data, then branch, then store
  const providerAvatar =
    (userProvider
      ? getAvatarUrl(userProvider as { avatarFile?: { fileUrl: string } | null })
      : null) ||
    getAvatarUrl(provider) ||
    (selectedProvider ? getAvatarUrl(selectedProvider) : null);
                ...
                <div className="relative flex-shrink-0">
                  {providerAvatar ? (
                    ...
                    <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-400 to-primary-600 shadow-xl">
                      {(selectedProvider?.serviceType || provider.serviceType) === 'company' ? (
          ...
          <div className="glass-light rounded-2xl p-6 lg:p-8">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === 'info' && <ProviderEditor providerId={providerId} />}
